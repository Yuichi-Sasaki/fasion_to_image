...
from transformers import CLIPTextModel, CLIPTokenizer, CLIPVisionModel, CLIPModel, CLIPProcessor, CLIPFeatureExtractor
...
def parse_args():
    ...
    parser.add_argument(
        "--image_column", type=str, default="pixel_values", help="The column of the dataset containing an image."
    )
    parser.add_argument(
        "--caption_column",
        type=str,
        #default="text",
        default="ref_pixel_values",
...
def main():
    ...
    converter = ConverterModel(ConverterConfig())
    #convert_layer = nn.Linear(1024,768)
    ...
    def preprocess_train(examples):
        ...
        images_ref = [image.convert("RGB") for image in examples["ref_pixel_values"]]
        examples["pixel_values"] = [train_transforms(image) for image in images]
        examples["ref_pixel_values"] = [train_transforms_ref(image) for image in images_ref]
    ...
    #accelerator.register_for_checkpointing(vision_encoder)
    #accelerator.register_for_checkpointing(converter)
